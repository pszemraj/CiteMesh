...
_ARXIV_IDENTIFIER_PATTERN = re.compile(
    r"^(?:"
    r"(?P<new_year>\d{2})(?P<new_month>\d{2})\.(?P<new_sequence>\d{4,5})"
    r"|[a-z][a-z-]*(?:\.[a-z-]+)?/"
    r"(?P<old_year>\d{2})(?P<old_month>\d{2})(?P<old_sequence>\d{3})"
    r")(?:v\d+)?$",
    re.IGNORECASE,
)
...
def arxiv_id_chronology_key(raw_id: Any) -> tuple[int, int, int] | None:
    ...
    if text.startswith("arxiv:"):
        text = text[len("arxiv:") :]
    match = _ARXIV_IDENTIFIER_PATTERN.fullmatch(text)
    if match is None:
        return None
    style = "new" if match.group("new_year") is not None else "old"
    year_token = int(match.group(f"{style}_year"))
    month = int(match.group(f"{style}_month"))
    sequence = int(match.group(f"{style}_sequence"))
