...
- Added `citemesh search --local`: semantic search over the locally cached embeddings (candidate vectors accumulated across builds, or a hydrated corpus) via the cache-native search API. The query is encoded in the model's query prompt space and ranked with cosine scores; no Semantic Scholar traffic. Targets the same cache namespace a flagless build writes to, honoring `config.toml` defaults, with `--model`/`--device` overrides.
