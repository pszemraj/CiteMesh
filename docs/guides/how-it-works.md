...
Encoding is the expensive part, so every vector is persisted: the `data/embedding_cache/` module writes an `embeddings/` directory under the cache root, holding metadata in SQLite and vectors in a resizable HDF5 matrix, one pair per namespace.

The namespace fingerprint is what makes reuse safe: it joins every token that could change what a vector *means*, from the model and its `artifact` identity through representation, truncate dim, formatter, and storage contract. `artifact` is immutable — a resolved commit SHA, else a digest over the inference-artifact manifest — so mutating a checkpoint in place selects a different cache instead of poisoning the old one. There is no device or compute-dtype token: CPU, CUDA, and MPS share a namespace when every other contract matches, so an index built on a bf16 GPU is portable to an fp32 laptop.
