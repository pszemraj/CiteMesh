...
- `arxiv-corpus` hydrates a local arXiv abstract corpus from HuggingFace (`librarian-bots/arxiv-metadata-snapshot` by default) and searches that. It surfaces work no citation path would reach, at the cost of the `datasets` extra and substantially more cold-cache time. `--dataset-source` and `--dataset-split` select another repository or split; `--corpus-size N` sizes the initial hydration to the N newest submissions after scanning the split, and later runs can grow that cached corpus past it. Hydration, resumption, and storage behavior are in [Caching & Data](caching.md).
...
- `--max-semantic` limits semantic-*only* additions, not overlap papers that both branches found, and it is a ceiling rather than a reservation. The union is one ranked list, so once a seed's references and citations fill `--max-papers`, a semantic-only candidate must out-score citation-derived ones, which also carry a small provenance bonus; a heavily cited seed can therefore yield zero semantic-only papers in `candidates` mode. Lower `--max-citations` or use `--semantic-source arxiv-corpus` when semantic recall matters. `--max-semantic 0` falls back to the citation scorer and threshold while keeping hybrid's degree cap.
