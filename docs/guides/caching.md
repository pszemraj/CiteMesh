...
Interrupted capped runs (`--corpus-size`) and sliced splits also preserve completed vectors when the hydration metadata still matches. Resume scans the selected corpus again and encodes only paper IDs missing from the cache before marking hydration complete.
