...
Semantic Scholar calls retry automatically with exponential backoff - up to 30 attempts per operation with no overall deadline; a collection stops issuing later Semantic Scholar requests after one source exhausts its retries. The full policy is under [Troubleshooting](#troubleshooting).
...
- **Partial Semantic Scholar outage**: CiteMesh continues only when at least one requested neighborhood source completed. After one source exhausts its retries, the collection skips later Semantic Scholar requests and records their sources as `unavailable`. Earlier `complete` or `empty` evidence remains usable, and persisted reference-cache hits can still hydrate papers. Export metadata records each attempted source as `complete`, `empty`, or `unavailable`; a total source outage exits nonzero instead of producing a plausible seed-only graph. A later independent collection starts a fresh retry attempt.
