#!/usr/bin/env python3
"""
reference tool-style visualization implementing the correct algorithm
...
class reference toolBuilder:
    """Build a reference tool-style similarity graph."""
    ...
    def build_similarity_graph(self, paper_id: str, max_papers: int = 40) -> nx.Graph:
        """
        Build similarity graph following reference tool algorithm.
        
        Phase 1: Collect candidate papers
        Phase 2: Select by similarity  
        Phase 3: Build graph with similarity edges
        """
        print(f"Building reference tool graph for {paper_id}")
...
def compute_reference_tool_layout(graph: nx.Graph, seed_id: str) -> Dict:
    """
    Compute force-directed layout matching reference tool style.
...
def visualize_reference_tool(graph: nx.Graph, output_path: Path, seed_id: str):
    """Create reference tool-style visualization."""
    
    # Compute layout
    pos = compute_reference_tool_layout(graph, seed_id)
    ...
    ax.set_title(f"reference tool: {seed_title}", fontsize=14, pad=20)
...
def main():
    parser = argparse.ArgumentParser(description="Create reference tool-style visualization")
    parser.add_argument("paper_id", help="Paper ID (DOI, arXiv ID, or S2 ID)")
    parser.add_argument("-o", "--output", type=Path, default=Path("out/reference tool.png"))
    parser.add_argument("--max-papers", type=int, default=40, help="Maximum papers to show")
    args = parser.parse_args()
    
    # Build graph
    builder = reference toolBuilder()
    graph = builder.build_similarity_graph(args.paper_id, max_papers=args.max_papers)
    
    # Visualize
    visualize_reference_tool(graph, args.output, builder.seed_paper_id)
