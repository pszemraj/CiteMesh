...
def test_hybrid():
    ...
    try:
        citations = semantic_scholar.get_paper_citations(
            seed.paperId, limit=5, fields=["title", "year", "abstract", "paperId"]
        )
    ...
    texts = [
        f"{papers[pid]['title']}. {papers[pid]['abstract'][:500]}" for pid in paper_ids
    ]
    ...
    for i, pid in enumerate(paper_ids):
        paper = papers[pid]
        graph.add_node(
            pid, title=paper["title"], year=paper["year"], is_seed=paper["is_seed"]
        )
